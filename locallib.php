...
/**
 * Helper functions for tb_a_courses block
 *
 ...
 */
function block_tb_a_courses_get_overviews($courses) {
    $htmlarray = array();
...
/**
 * Sets user preference for maximum courses to be displayed in tb_a_courses block
 *
 * @param int $number maximum courses which should be visible
 */
function block_tb_a_courses_update_mynumber($number) {
    set_user_preference('tb_a_courses_number_of_courses', $number);
}

/**
 * Sets user course sorting preference in tb_a_courses block
 *
 * @param array $sortorder list of course ids
 */
function block_tb_a_courses_update_myorder($sortorder) {
    ...
    set_user_preference('tb_a_courses_course_sortorder', $value);
}

/**
 * Gets user course sorting preference in tb_a_courses block
 *
 * @return array list of course ids
 */
function block_tb_a_courses_get_myorder() {
    if ($value = get_user_preferences('tb_a_courses_course_sortorder')) {
        return explode(',', $value);
    }
    // If preference was not found, look in the old location and convert if found.
    $order = array();
    if ($value = get_user_preferences('tb_a_courses_course_order')) {
        $order = unserialize_array($value);
        block_tb_a_courses_update_myorder($order);
        unset_user_preference('tb_a_courses_course_order');
    }
    return $order;
}
 ...
 */
function block_tb_a_courses_get_child_shortnames($courseid) {
    global $DB;
    ...
    if ($results = $DB->get_records_sql($sql, $params)) {
        ...
        if ($total > 10) {
            ...
            if ($diff > 1) {
                $suffix = get_string('shortnamesufixprural', 'block_tb_a_courses', $diff);
            } else {
                $suffix = get_string('shortnamesufixsingular', 'block_tb_a_courses', $diff);
            }
        }
        $shortnames = get_string('shortnameprefix', 'block_tb_a_courses', implode('; ', $shortnames));
        $shortnames .= $suffix;
    }
...
/**
 * Returns maximum number of courses which will be displayed in tb_a_courses block
 *
...
function block_tb_a_courses_get_sorted_courses($showallcourses = false, $categoryid = 1) {
    ...
    $order = block_tb_a_courses_get_myorder();
