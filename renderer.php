 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_tb_a_courses_renderer extends plugin_renderer_base {
    ...
    public function tb_a_courses($courses, $config) {
        ...
        if ($this->page->user_is_editing() && (count($courses) > 1)) {
            $userediting = true;
            $this->page->requires->js_init_call('M.block_tb_a_courses.add_handles');
        ...
        if ($ismovingcourse) {
            ...
            $html .= get_string('movingcourse', 'block_tb_a_courses', $a);
            $html .= $this->output->box_end();

            $moveurl = new moodle_url('/blocks/tb_a_courses/move.php',
            ...
            $movetofirsticon = html_writer::empty_tag('img',
                array('src' => $moveicon,
                    'alt' => get_string('movetofirst', 'block_tb_a_courses', $courses[$movingcourseid]->fullname),
        ...
        if($config->available_showasslider == 1){    
            $html .= html_writer::start_div('tb_a_courses_list owl-carousel owl-theme');
        }else{
            $html .= html_writer::start_div('tb_a_courses_list');
        }  
        foreach ($courses as $key => $course) {
            ...
            if ($userediting && !$ismovingcourse) {
                $moveicon = html_writer::empty_tag('img',
                    array('src' => $moveicon->out(false),
                        'alt' => get_string('movecourse', 'block_tb_a_courses', $course->fullname),
            ...
            if ($ismovingcourse) {
                ...
                $movehereicon = html_writer::empty_tag('img',
                    array('src' => $movehere,
                        'alt' => get_string('moveafterhere', 'block_tb_a_courses', $a),
                        'title' => get_string('movehere')));
    ...
    protected function activity_display($cid, $overview) {
        ...
        foreach (array_keys($overview) as $module) {
            ...
            if (get_string_manager()->string_exists("activityoverview", $module)) {
                ...
                $icontext .= get_string("activityoverview", 'block_tb_a_courses', $modulename);
            }
    ...
    public function editing_bar_head($max = 0) {
        ...
        $options = array('0' => get_string('alwaysshowall', 'block_tb_a_courses'));
        for ($i = 1; $i <= $max; $i++) {
            $options[$i] = $i;
        }
    ...
    protected function collapsible_region_start($classes, $id, $caption, $userpref = '', $default = false) {
        ...
        $this->page->requires->js_init_call('M.block_tb_a_courses.collapsible', array($id, $userpref, get_string('clicktohideshow')));
    ...
    public function welcome_area($msgcount) {
        ...
        $output .= $this->output->heading(get_string('welcome', 'block_tb_a_courses', $USER->firstname));

        if (!empty($CFG->messaging)) {
            $plural = 's';
            if ($msgcount > 0) {
                $output .= get_string('youhavemessages', 'block_tb_a_courses', $msgcount);
                if ($msgcount == 1) {
                    $plural = '';
                }
            } else {
                $output .= get_string('youhavenomessages', 'block_tb_a_courses');
            }
            $output .= html_writer::link(new moodle_url('/message/index.php'),
                get_string('message' . $plural, 'block_tb_a_courses'));
        }
        $output .= $this->output->box_end();
    ...
    public function course_image_defaults($config) {
        ...
        if (method_exists($this->output, 'image_url')) {
            // Use the new method.
            $default = $this->output->image_url('default', 'block_tb_a_courses');
        } else {
            // Still a pre Moodle 3.3 release. Use pix_url because image_url doesn't exist yet.
